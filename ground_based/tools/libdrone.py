...
# rough interface reflectivities
    ...
    sh     = sv*(np.exp(-hG*cosd(tetain)**nGh)) # H pol.
    sv     = sh*(np.exp(-hG*cosd(tetain)**nGv)) # V pol.
    return sv,sh
...
def getMapParams(latr,lonr):
    ...
    g = GoogleVisibleMap(x=[maplonmin, maplonmax], y=[maplatmin, maplatmax],scale=2,maptype='satellite')  # try out also: 'terrain'
...
def plotscattermap(sm,x,y,txminp,txmaxp,tyminp,tymaxp,plotvar,varmin,varmax,fname,varname,unitstr,fpath,savefigbool):
    ...
    scat = ax1.scatter(x, y, c=plotvar, s=30, edgecolors='none', linewidths=1,cmap=cm,alpha=1.0,vmin=varmin,vmax=varmax)
...
def plotcontmap(xi,yi,zi,bool2b1,sm,varmin,varmax,txminp,txmaxp,tyminp,tymaxp,fname,varname,unitstr,darkmode,fpath,savefigbool):
    ...
    #        c.set_edgecolor("face",alpha=0.7)
        ...
        sm.visualize(ax=ax1)  # plot it
        #plt.sca(ax1)
        ax1.set_xlim((txminp,txmaxp))
        ax1.set_ylim((tymaxp,tyminp))
        plt.tight_layout()
        #plt.sca(ax2)
